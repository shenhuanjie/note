...
在和 [`output.library`](https://webpack.docschina.org/configuration/output/#output-library) 和 [`output.libraryTarget`](https://webpack.docschina.org/configuration/output/#output-librarytarget) 一起使用时，此选项允许用户向导出容器(export wrapper)中插入注释。要为 `libraryTarget` 每种类型都插入相同的注释，将 `auxiliaryComment` 设置为一个字符串：
...
此选项决定了非入口(non-entry) chunk 文件的名称。有关可取的值的详细信息，请查看 [`output.filename`](https://webpack.docschina.org/configuration/output/#output-filename) 选项。
...
只用于 [`target`](https://webpack.docschina.org/configuration/target) 是 web，使用了通过 script 标签的 JSONP 来按需加载 chunk。
...
查看 [`output.devtoolModuleFilenameTemplate`](https://webpack.docschina.org/configuration/output/#output-devtoolmodulefilenametemplate)。
...
此选项仅在 「[`devtool`](https://webpack.docschina.org/configuration/devtool) 使用了需要模块名称的选项」时使用。
...
如果多个模块产生相同的名称，使用 [`output.devtoolFallbackModuleFilenameTemplate`](https://webpack.docschina.org/configuration/output/#output-devtoolfallbackmodulefilenametemplate) 来代替这些模块。
...
此选项确定 [`output.devtoolModuleFilenameTemplate`](https://webpack.docschina.org/configuration/output/#output-devtoolmodulefilenametemplate) 使用的模块名称空间。未指定时的默认值为：[`output.library`](https://webpack.docschina.org/configuration/output/#output-library)。在加载多个通过 webpack 构建的 library 时，用于防止 source map 中源文件路径冲突。
...
此选项决定了每个输出 bundle 的名称。这些 bundle 将写入到 [`output.path`](https://webpack.docschina.org/configuration/output/#output-path) 选项指定的目录下。

对于单个[`入口`](https://webpack.docschina.org/configuration/entry-context#entry)起点，filename 会是一个静态名称。
...
请确保已阅读过[指南 - 缓存](https://webpack.docschina.org/guides/caching)的详细信息。这里涉及更多步骤，不仅仅是设置此选项。
...
自定义热更新 chunk 的文件名。可选的值的详细信息，请查看 [`output.filename`](https://webpack.docschina.org/configuration/output/#output-filename) 选项。
...
只在 [`target`](https://webpack.docschina.org/configuration/target) 是 web 时使用，用于加载热更新(hot update)的 JSONP 函数。

JSONP 函数用于异步加载(async load)热更新(hot-update) chunk。

详细请查看 [`output.jsonpFunction`](https://webpack.docschina.org/configuration/output/#output-jsonpfunction)。
...
自定义热更新的主文件名(main filename)。可选的值的详细信息，请查看 [`output.filename`](https://webpack.docschina.org/configuration/output/#output-filename) 选项
...
只在 [`target`](https://webpack.docschina.org/configuration/target) 是 web 时使用，用于按需加载(load on-demand) chunk 的 JSONP 函数。
...
如果使用了 [`output.library`](https://webpack.docschina.org/configuration/output/#output-library) 选项，library 名称时自动追加的。
...
`output.library` 的值的作用，取决于[`output.libraryTarget`](https://webpack.docschina.org/configuration/output/#output-librarytarget) 选项的值；完整的详细信息请查阅该章节。注意，`output.libraryTarget` 的默认选项是 `var`，所以如果使用以下配置选项：
...
配置如何暴露 library。可以使用下面的选项中的任意一个。注意，此选项与分配给 [`output.library`](https://webpack.docschina.org/configuration/output/#output-library) 的值一同使用。对于下面的所有示例，都假定将 `output.library` 的值配置为 `MyLibrary`。
...
注意，省略 `library` 会导致将入口起点返回的所有属性，直接赋值给 root 对象，就像[对象分配章节](https://webpack.docschina.org/configuration/output/#expose-via-object-assignment)。例如：
...
你的 library 的依赖将由 [`externals`](https://webpack.docschina.org/configuration/externals/) 配置定义。
...
注意，`[hash]` 在参数中被替换为编译过程(compilation)的 hash。详细信息请查看[指南 - 缓存](https://webpack.docschina.org/guides/caching)。
...
告知 webpack 在 bundle 中引入「所包含模块信息」的相关注释。此选项在 `development` [模式](https://webpack.docschina.org/concepts/mode/)时的默认值是 `true`，而在 `production` [模式](https://webpack.docschina.org/concepts/mode/)时的默认值是 `false`。

> 对于在开发环境(development)下阅读生成代码时，虽然通过这些注释可以提供非常有用的数据信息，但在生产环境(production)下，**不应该**使用。
...
注意，参数中的 `[hash]` 将会被替换为编译过程(compilation) 的 hash。详细信息请查看[指南 - 缓存](https://webpack.docschina.org/guides/caching)。
...
此选项会向硬盘写入一个输出文件，只在 [`devtool`](https://webpack.docschina.org/configuration/devtool) 启用了 SourceMap 选项时才使用。

配置 source map 的命名方式。默认使用 `"[file].map"`。

可以使用 [#output-filename](https://webpack.docschina.org/configuration/output/#output-filename) 中的 `[name]`, `[id]`, `[hash]` 和 `[chunkhash]` 替换符号。除此之外，还可以使用以下替换符号。`[file]` 占位符会被替换为原始文件的文件名。我们建议**只使用 [file] 占位符**，因为其他占位符在非 chunk 文件(non-chunk files)生成的 SourceMap 时不起作用。
...
模块 [basename](https://nodejs.org/api/path.html#path_path_basename_path_ext)
