...
`designWidth` 用来设置设计稿尺寸，关于这一部分的配置说明请见[设计稿及尺寸单位](https://nervjs.github.io/taro/docs/size.html)这一章节。
...
dll 开关，开启后将使用`dllPlugin`把内置的部分依赖库打包为单独的dll文件， 某种程度上可以减少首屏单个文件体积。 dev 状态默认 **关**，prod 状态默认 **开**。
